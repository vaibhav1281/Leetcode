...
// The function checks if `i` is greater than `n`. If it is, it returns 0, which serves as the base case to stop the recursion. Otherwise, it prints "Name" and calls itself with `i + 1`, which is the recursive case that continues to call the function until the base case is reached.  
